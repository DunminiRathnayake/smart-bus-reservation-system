      ...
      <header className="sticky top-0 z-50 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-6">
            <Link to="/search-bus" className="hover:text-emerald-400 transition-colors">Search Buses</Link>
            {user ? (
              <>
                ...
                {user.role === 'ROLE_PASSENGER' && (
                  <Link to="/dashboard" className="hover:text-emerald-400 transition-colors flex items-center gap-1.5">
                    <LayoutDashboard className="h-4 w-4" /> Dashboard
                  </Link>
                )}
                
                <div className="flex items-center space-x-4 border-l border-slate-800 pl-6">
                  <Link to="/profile" className="flex items-center gap-1.5 hover:text-emerald-400 transition-colors">
              ...
              <div className="flex items-center space-x-4">
                ...
                <Link
                  to="/register"
                  className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 rounded-md font-semibold text-sm transition-colors text-slate-950 shadow-md shadow-emerald-500/10"
                >
                  Register
                </Link>
              </div>
